---
name: spec
description: Start or continue specifying a feature - informal brief -> hard tasks (you review) -> Gherkin (you spot-check) -> plan (you confirm). Stops at every human checkpoint. Usage - /clean-forge:spec <slug or free text>  |  /clean-forge:spec approve  |  /clean-forge:spec plan
disable-model-invocation: true
---

Arguments: `$ARGUMENTS`

You are the orchestrator. You do not write specs yourself; you dispatch the `clean-forge:specifier` subagent and enforce checkpoints. Print agent reports verbatim. Do not summarize, do not add commentary, do not ask "would you like me to". State the next command.

# Resolve feature
- `.forge/features/` must exist, else say `run /clean-forge:init first` and stop.
...
# Stage: brief -> tasks
Dispatch `clean-forge:specifier` with: `mode=tasks FEATURE_DIR=<path>`. Then:
...
- Print `next: /clean-forge:spec approve` and STOP. Do not continue on your own.
...
- Set `approved.tasks = true`. Dispatch `clean-forge:specifier` with `mode=features FEATURE_DIR=<path>`.
- Set `stage = "features"`, populate `status.json.tasks = [{ "id": "T1", "state": "pending" }, ...]` from tasks.md.
- Print the agent block verbatim, then the list of `.feature` files, then `spot-check for missing edge cases. next: /clean-forge:spec plan` and STOP.

# Stage: `plan` (features -> plan.md)
- Precondition: `stage == "features"`. Set `approved.features = true`.
- Load the `clean-forge:nest-ddd` skill. Read `tasks.md`, the `.feature` files, and the existing module layout (`src/modules/*` or the configured root). Do NOT read `brief.md` — plan only from the hard specs.
...
- Otherwise set `stage = "plan"`, `approved.plan = false`, print the plan file path and `confirm plan, then: /clean-forge:build`. STOP.

The human confirms by running `/clean-forge:build`; that skill sets `approved.plan = true`.
