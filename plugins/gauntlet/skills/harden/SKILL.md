---
name: harden
description: Run the hardener on a built feature - Stryker mutation testing, Gherkin mutation, kill survivors with tests, final full gate. Unattended; CPU-heavy. Usage - /gauntlet:harden [feature-dir]
...
# Preconditions
- Resolve feature dir: explicit arg, else the single feature with `stage == "built"`. If none, say `nothing built — run /gauntlet:build` and stop.
- Working tree clean (built features are committed). Else stop.
- `touch .forge/gate-on`.

# Run
Print `→ hardener`. Dispatch `gauntlet:hardener` with `FEATURE_DIR=<path>` and the list of touched files gathered from all coder handoffs (`files:` lines).
